...
export default async function AdminPage() {
  ...
  const { data: footerSettings } = await supabase.from('footer_settings').select('*').single()
  const { data: privacyPolicy } = await supabase.from('privacy_policy').select('*').single()
  ...
  return (
    <div style={{ padding: '48px 24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <AdminDashboardClient 
        ...
        initialPrivacy={privacyPolicy || null}
      />
    </div>
  )
}
